...
class Datatable_gugus extends MX_Controller{
	...
	public function datatable($data = "")
	{
		...
		if(isset($data["video-location"])){
			$this->video_path = $data["video-location"];
		}
		...
		foreach($theTable as $key => $value){
            ...
            foreach($show_table as $evelop => $variable){
            	...
            	if (preg_match("/{$perkalian}/i", $variable)) {
                	...
                	$child[] = '
		                ...
		                <script>
							$("body").on("click", "[data-target'.$data1.$mynom.']", function(event){
								event.preventDefault();
								$("#'.$modalname.$mynom.'").modal("show");
								$("#modal-content'.$modalname.$mynom.'").html(`'.htmlspecialchars_decode($value->$data1).'`);
								$("#modal-title").html(`'.$data1.'`);
							})
		                </script>
                	';
            	}elseif(preg_match("/{$video}/i", $variable)) {
            		$datavar = explode(":", $variable);
            		$data1 = $datavar[1];
                	$child[] = '
						<button type="button" class="btn btn-primary" data-toggle="modal" data-target'.$data1.$mynom.'>
		                ...
		                <script>
							$("body").on("click", "[data-target'.$data1.$mynom.']", function(event){
								event.preventDefault();
								$("#'.$modalname.$mynom.'").modal("show");
								$("#modal-content'.$modalname.$mynom.'").html(`
									<video width="100%" controls>
									  <source src="'.$this->video_path.$value->$data1.'" type="video/3gp">
									  <source src="'.$this->video_path.$value->$data1.'" type="video/mp4">
									  <source src="'.$this->video_path.$value->$data1.'" type="video/ogg">
            		...
            		$modalname = $variable;
                	$child[] = $childs.'
						<div class="modal fade" id="'.$variable.$mynom.'">
					        <div class="modal-dialog modal-lg">
					          <div class="modal-content">
					            ...
					            <div class="modal-body" id="modal-content'.$variable.$mynom.'">
            ...
            if ($this->action == "standart") {
	            ...
	            <center>
	            	...
	            	<button data-id='".$value->$key_id."' class='btn btn-success edit'> <i class='fas fa-edit'></i></button>
	            	<button data-id='".$value->$key_id."' class='btn btn-danger delete'> <i class='fas fa-trash'></i></button>
	            </center>
	            ";
            }
